...
function allpay_cvs_link($params) {
    ...
    $log = Capsule::table('tblactivitylog')
            ->where('description', 'like', 'allpay_cvs:{"'.$params['invoiceid'].'":%')
            ->orderBy('id', 'desc')
            ->first();
    if ($log) {
    ...
    $transaction->ReturnURL = $systemurl.'/modules/gateways/callback/allpay_cvs.php';
    $transaction->PaymentInfoURL = $systemurl.'/modules/gateways/callback/allpay_cvs_info.php';
